...
use crate::wallet::{batch, Wallet};
use crate::wallet::batch::Plan;
...
//     }
    ...
    async fn wallet_btc_utxos(
        ...
        task::block_in_place(|| {
            ...
            for btc_utxo in btc_utxos {
                if (btc_utxo.1).inscriptions.is_empty() && (btc_utxo.1).runes.is_empty()
                    && btc_utxo.1.indexed
                    && !btc_utxo.1.spent
                    && btc_utxo.1.sat_ranges.is_some()
                    && !btc_utxo.1.sat_ranges.as_ref().unwrap().is_empty() {
                    utxos.push(btc_utxo);
                }
            }
            // let data = btc_utxos.map(|utxos| {
            //
            //     utxos.0
            //     // utxos[1]
            //
