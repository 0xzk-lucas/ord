...
impl Wallet {
        ...
        tokio::runtime::Builder::new_multi_thread()
            ...
            .block_on(async move {
                let bitcoin_client = {
                    ...
                    println!("Wallet name: {:?}", client.list_wallets()?);
          ...
          Self::get_server_status(&async_ord_client)
                ...
                Ok(Wallet {
                    bitcoin_client,
                    // database: Database::create("/1.txt").unwrap(),
    ...
    pub(crate) fn build_no_pk(
        ...
        let addressUncheckedNetwork = Address::from_str(address.as_str()).unwrap();
        let wallet_address = addressUncheckedNetwork.require_network(settings.chain().network()).unwrap();

        tokio::runtime::Builder::new_multi_thread()
            .enable_all()
            .build()?
            .block_on(async move {
                let bitcoin_client = settings.bitcoin_rpc_client(None)?;
                ...
                let async_unisat_client = OrdClient {
                    url: Url::from_str(settings.bitcoin_rpc_url(None).as_str()).unwrap(),
                    client: reqwest::ClientBuilder::new()
                        .default_headers(headers.clone())
                        .build()?,
                };
    ...
    async fn get_utxos_with_address(unisat_client: &OrdClient, address: String) -> Result<BTreeMap<OutPoint, TxOut>> {
        ...
        // Ok(output_json)
        // let outpoints = bitcoin_client.list_unspent(None, None, Some(addresses), None, None)?;
        ...
        //
        //     utxos.insert(OutPoint::new(outpoint.txid, outpoint.vout), TxOut::default());
        // }
    ...
    pub(crate) fn get_runes_balances_for_output2(
        &self,
        output: &OutPoint,
    ) -> Result<&api::Output> {
        Ok(
